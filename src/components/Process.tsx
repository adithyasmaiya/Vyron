...
export default function Process() {
  ...
  return (
    <section id="process" className="relative scroll-mt-20 overflow-hidden">
      {/* Desktop: Pinned Horizontal Execution Pipeline driven by page scroll */}
      <div
        ref={wrapRef}
      ...
      >
        ...
        <div className="relative mt-8 w-full overflow-hidden">
          ...
          >
            {steps.map((s) => (
              <article
                key={s.id}
                className="group relative flex w-[380px] shrink-0 flex-col"
              >
                {/* Pipeline Execution Rail & Telemetry Node */}
                <div className="mb-6 flex items-center gap-3">
                  <div className="relative flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-white/15 bg-[#090b12] transition-all duration-500 group-hover:border-electric group-hover:shadow-[0_0_12px_rgba(77,124,254,0.5)] group-[[data-active=true]]:border-electric group-[[data-active=true]]:shadow-[0_0_12px_rgba(77,124,254,0.5)]">
                    <span className="h-2 w-2 rounded-full bg-electric/50 transition-all duration-500 group-hover:scale-125 group-hover:bg-electric group-[[data-active=true]]:scale-125 group-[[data-active=true]]:bg-electric" />
                  </div>
                  ...
                  <span className="shrink-0 font-mono text-[10px] tracking-[0.25em] text-white/35 transition-colors duration-500 group-hover:text-electric group-[[data-active=true]]:text-electric">
                    PHASE_{s.code}
                  </span>
                </div>

                {/* Pipeline Stage Card Module */}
                <div className="relative flex h-full flex-col justify-between overflow-hidden rounded-3xl border border-white/[0.09] bg-[#0c0d14]/90 p-8 backdrop-blur-xl transition-all duration-500 group-hover:-translate-y-1.5 group-hover:border-electric/40 group-hover:bg-[#0e101b]/95 group-hover:shadow-[0_24px_60px_-20px_rgba(77,124,254,0.35)] group-[[data-active=true]]:border-electric/40 group-[[data-active=true]]:bg-[#0e101b]/95 group-[[data-active=true]]:shadow-[0_20px_50px_-20px_rgba(77,124,254,0.25)]">
                  ...
                  <div>
                    ...
                    <div className="flex items-baseline justify-between">
                      <span className="select-none font-mono text-5xl font-extrabold tracking-tighter text-white/20 transition-colors duration-500 group-hover:text-white/35 group-[[data-active=true]]:text-electric/40 sm:text-6xl">
                        {s.code}
                      </span>
                      <span className="rounded-md border border-white/[0.08] bg-white/[0.03] px-2.5 py-1 font-mono text-[10px] tracking-[0.25em] text-white/50 uppercase backdrop-blur-sm transition-colors duration-300 group-hover:border-electric/30 group-hover:text-white/80 group-[[data-active=true]]:border-electric/30 group-[[data-active=true]]:text-white/80">
                        {s.phase.toUpperCase()}
                      </span>
                    </div>

                    {/* Step Title & Narrative */}
                    <h3 className="mt-5 font-display text-2xl font-semibold tracking-tight text-white transition-colors duration-300">
                      {s.name}
                    </h3>
                    <p className="mt-3 text-sm leading-relaxed text-white/60 transition-colors duration-300 group-hover:text-white/75 group-[[data-active=true]]:text-white/75">
                      {s.description}
                    </p>
                  </div>
                  ...
                  <div className="mt-8 border-t border-white/[0.08] pt-5">
                    ...
                    <ul className="mt-3.5 space-y-2.5">
                      {s.outputs.map((o) => (
                        <li
                          key={o}
                          className="flex items-center gap-2.5 text-[13px] text-white/70 transition-colors duration-300 group-hover:text-white/85"
                        >
        ...
        <div className="mx-auto flex w-full max-w-7xl items-center gap-6 px-8 pt-6">
          <div className="flex items-center gap-2 font-mono text-[10px] tracking-[0.3em] text-white/45">
      ...
      <div className="px-5 py-24 sm:px-8 lg:hidden">
        ...
        >
          ...
          <div className="mt-3 flex items-center justify-between font-mono text-[10px] tracking-[0.25em] text-white/35 uppercase">
          ...
          {steps.map((s) => (
            ...
            >
              ...
              <div className="mb-4 flex items-center gap-3">
                <div className="relative flex h-4 w-4 shrink-0 items-center justify-center rounded-full border border-white/15 bg-[#090b12]">
                  <span className="h-1.5 w-1.5 rounded-full bg-electric" />
                </div>
                <div className="relative h-px w-full overflow-hidden bg-white/[0.08]">
                  <div className="h-full w-full bg-gradient-to-r from-electric/50 via-white/20 to-white/[0.04]" />
                </div>
                <span className="shrink-0 font-mono text-[9px] tracking-[0.2em] text-white/35">
                  PHASE_{s.code}
                </span>
              </div>
              ...
              <div className="relative flex h-full flex-col justify-between overflow-hidden rounded-3xl border border-white/[0.09] bg-[#0c0d14]/90 p-7 backdrop-blur-xl">
                ...
                <div>
                  <div className="flex items-baseline justify-between">
                    <span className="font-mono text-5xl font-extrabold tracking-tighter text-white/20 select-none">
                      {s.code}
                    </span>
                    <span className="rounded-md border border-white/[0.08] bg-white/[0.03] px-2.5 py-1 font-mono text-[10px] tracking-[0.25em] text-white/50 uppercase">
                      {s.phase.toUpperCase()}
                    </span>
                  </div>
                  ...
                  <ul className="mt-3 space-y-2">
                    {s.outputs.map((o) => (
                      <li key={o} className="flex items-center gap-2.5 text-[13px] text-white/70">
                        <span className="h-1.5 w-1.5 shrink-0 rounded-[1px] bg-electric" />
                        <span>{o}</span>
                      </li>
                    ))}
                  </ul>
